...
static void LoadProxyDialog() {
    ...
    if (f.is_open()) {
        ...
        g_proxy_lines = {
            ...
            "",
            "[Economy & Drops]",
            "/fd : Toggle fast drop mode (auto-confirms drop dialogs)",
            "/dropall : Drop all items from inventory",
            "/dropall [amt] : Drop specified amount of each item",
            "/dpos : Set drop position where you stand (like pos1-4)",
            "/dropat [amt] : Teleport to /dpos position and drop items",
            "/daw : Drop all WLs, DLs, and BGLs from inventory",
            "/dw [amt] : Quick drop World Locks",
            "/dd [amt] : Quick drop Diamond Locks",
            "/dbgl [amt] : Quick drop Blue Gem Locks",
            ...
            "/host, /qq, /reme, /ignorecsn, /ignorecsnchat : Casino tools",
            "",
            "[Automation & Utility]",
...
static void ApplyVinProxyTheme() {
...
void RunImGuiApp() {
    ...
    if (!hIcon) {
        hIcon = (HICON)LoadImage(nullptr, _T("app.ico"), IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE);
    }
    if (!hIcon) {
        hIcon = (HICON)LoadImage(nullptr, _T("resources/app.ico"), IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE);
    }

    WNDCLASSEX wc = {
        sizeof(wc), CS_CLASSDC, WndProc, 0, 0,
        GetModuleHandle(nullptr), hIcon, nullptr, nullptr, nullptr,
        _T("VinProxyImGui"), hIcon
    };
    RegisterClassEx(&wc);
