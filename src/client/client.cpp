    ...
    , player_{ nullptr }
{
    ...
    core_->get_event_dispatcher().appendListener(
        ...
        [&](const core::EventConnection& evt)
        {
            ...
            std::string connect_target = config.get<std::string>("server.address");
            uint16_t connect_port = static_cast<uint16_t>(config.get<unsigned int>("server.port"));
            auto* web_ext = core_->query_extension<IWebServerExtension>();
            if (web_ext && !web_ext->get_address().empty() && web_ext->get_port() != 65535) {
                connect_target = web_ext->get_address();
                connect_port = web_ext->get_port();
            }

            spdlog::info("[CLIENT] Connecting to subserver {}:{}", connect_target, connect_port);
            std::ignore = connect(connect_target, connect_port);
            ...
            try {
                ...
                if (variant.deserialize(ext_data)) {
                    ...
                    if (variants.size() >= 2) {
                        ...
                        if (function_name == "OnSpawn") {
                        ...
                        else if (function_name.find("OnSuperMainStartAcceptLogonHrd") != std::string::npos) {
                            if (variants.size() >= 2) {
                                ...
                                if (p_hash != 0) {
                                    ...
                                    spdlog::info("\033[32m[ItemsDatPatcher] Replaced items.dat hash in OnSuperMain: {} -> {}\033[0m", s_hash, p_hash);
                                }
                            }
                        }
                        else if (function_name == "OnSetClothing") {
                            ...
                            if (local_player.netID > 0 && game_update_packet.net_id == local_player.netID) {
                                if (variant.size() >= 5) {
                                    glm::vec3 v0 = variant.get<glm::vec3>(1);
                                    glm::vec3 v1 = variant.get<glm::vec3>(2);
                                    glm::vec3 v2 = variant.get<glm::vec3>(3);
                                    uint32_t skin = variant.get<uint32_t>(4);
                                    glm::vec3 v4 = (variant.size() >= 6) ? variant.get<glm::vec3>(5) : glm::vec3(0);
