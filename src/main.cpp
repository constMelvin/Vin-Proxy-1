...
#ifdef _WIN32
#include <tlhelp32.h>
static PROCESS_INFORMATION g_gt_process_info = {};
static bool g_gt_process_spawned = false;

static std::string get_growtopia_path(const std::string& custom_path) {
    ...
    if (GetEnvironmentVariableA("LOCALAPPDATA", local_appdata, MAX_PATH) > 0) {
        std::filesystem::path gt_path = std::filesystem::path(local_appdata) / "Growtopia" / "Growtopia.exe";
        if (std::filesystem::exists(gt_path)) {
            return gt_path.string();
        }
    }
    if (std::filesystem::exists("Growtopia.exe")) {
        return "Growtopia.exe";
    }
    return "";
}

static void launch_growtopia(core::Core* core) {
    if (!core) return;
    bool auto_launch = core->get_config().get<bool>("growtopia.auto_launch");
    if (!auto_launch) return;

    std::string custom_path = core->get_config().get<std::string>("growtopia.path");
    std::string gt_exe = get_growtopia_path(custom_path);

    if (gt_exe.empty()) {
        spdlog::warn("[Launcher] Could not locate Growtopia.exe automatically. Set 'growtopia.path' in config.json.");
        return;
    }

    spdlog::info("[Launcher] Auto-launching Growtopia from: {}", gt_exe);

    STARTUPINFOA si{};
    si.cb = sizeof(si);
    PROCESS_INFORMATION pi{};

    std::filesystem::path p(gt_exe);
    std::string work_dir = p.parent_path().string();
    ...
    if (CreateProcessA(
            ...
            &si,
            &pi)) {
        g_gt_process_info = pi;
        g_gt_process_spawned = true;
        spdlog::info("[Launcher] Growtopia launched successfully (PID: {})", pi.dwProcessId);
    } else {
        spdlog::error("[Launcher] Failed to launch Growtopia. Error code: {}", GetLastError());
    }
}

static void close_growtopia(core::Core* core = nullptr) {
    bool auto_close = true;
    if (core) {
        auto_close = core->get_config().get<bool>("growtopia.auto_close");
    }
    if (!auto_close) return;

    spdlog::info("[Launcher] Closing Growtopia process...");

    if (g_gt_process_spawned && g_gt_process_info.hProcess) {
        DWORD exit_code = 0;
        if (GetExitCodeProcess(g_gt_process_info.hProcess, &exit_code) && exit_code == STILL_ACTIVE) {
            TerminateProcess(g_gt_process_info.hProcess, 0);
            CloseHandle(g_gt_process_info.hProcess);
            CloseHandle(g_gt_process_info.hThread);
            g_gt_process_spawned = false;
            spdlog::info("[Launcher] Spawned Growtopia process terminated.");
            return;
        }
    }
    ...
    if (hSnap != INVALID_HANDLE_VALUE) {
        ...
        if (Process32First(hSnap, &pe)) {
            do {
                if (_stricmp(pe.szExeFile, "Growtopia.exe") == 0) {
                    HANDLE hProc = OpenProcess(PROCESS_TERMINATE, FALSE, pe.th32ProcessID);
                    if (hProc) {
                        TerminateProcess(hProc, 0);
                        CloseHandle(hProc);
                        spdlog::info("[Launcher] Terminated active Growtopia.exe (PID: {})", pe.th32ProcessID);
                    }
                }
...
#else
static void launch_growtopia(core::Core*) {}
static void close_growtopia(core::Core* = nullptr) {}
#endif
...
int main() {
    ...
    try {
        ...
        auto main_logger = std::make_shared<spdlog::logger>("VinProxy", sinks.begin(), sinks.end());
    ...
    try {
        ...
        // Ensure items.dat is patched with authentic weapon swing animations for Fist (Item 18) and visual weapons
        utils::ItemsDatPatcher::ensure_patched();

        launch_growtopia(&core);

        core.run();

        close_growtopia(&core);
