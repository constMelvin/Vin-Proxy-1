...
class ParserExtension final : public IParserExtension {
    ...
    mutable std::unordered_map<std::string, std::chrono::steady_clock::time_point> recent_fake_spins_;
    mutable std::unordered_map<int, std::chrono::steady_clock::time_point> recent_fake_spin_values_;
    mutable std::unordered_map<int, std::chrono::steady_clock::time_point> recent_playerchat_spin_values_;
    mutable std::unordered_map<uint32_t, int> last_spin_by_netid_;
    mutable std::unordered_map<std::string, std::chrono::steady_clock::time_point> pending_instant_spin_keys_;
    mutable std::unordered_map<int, std::chrono::steady_clock::time_point> pending_instant_spin_values_;
    mutable std::unordered_map<int, std::chrono::steady_clock::time_point> pending_fake_override_spin_values_;
    ...
    mutable std::vector<DeferredBubble> deferred_bubbles_;
...
public:
    void update_overhead_name_for(uint32_t netid, const std::string& base_name, const player::Player* target = nullptr) {
        ...
        } else if (core_ && core_->get_server() && core_->get_server()->get_player()) {
...
private:
    ...
    void handle_server_messages(const core::EventPacket& event) {
        ...
        try {
            if (variant.size() > 0) {
                ...
                auto extract_spin = [&](const std::string& raw, std::string& out_name, int& out_spin, std::string& out_key, uint32_t netid = 0) -> bool {
                ...
                auto get_reme_color_code = [&](int spin) -> std::string {
                    const int rem = sum_reme(spin);
                    if (rem == 8 || rem == 9 || rem == 0 || rem % 10 == 8 || rem % 10 == 9 || rem % 10 == 0) {
                        return "`2"; // Green
                    }
                    return "`w"; // White
                };
                auto find_netid_by_name = [&](const std::string& raw_name) -> uint32_t {
                    ...
                    if (!local_info.name.empty() && normalize_name(local_info.name) == target && local_info.netID > 0) {
                        return local_info.netID;
                    }
                ...
                auto is_player_chat_console = [&](const std::string& raw) -> bool {
                    ...
                    const size_t phrase_pos = lower.find("spun the wheel and got");
                    if (phrase_pos != std::string::npos) {
                        const std::string head = lower.substr(0, phrase_pos);
                        if (head.find(':') != std::string::npos || head.find('<') != std::string::npos || lower.find("player_chat=") != std::string::npos) {
                            return true; 
                        }
                        return false; 
                    }
                ...
                auto apply_last_suffix_for = [&](uint32_t netid, const std::string& base_name) {
                    update_overhead_name_for(netid, base_name, &event.get_target());
                };
                ...
                auto consume_pending_instant = [&](const std::string& key, int spin) -> bool {
                    const auto now = std::chrono::steady_clock::now();
                    constexpr auto kWindow = std::chrono::seconds(8);
                    bool matched = false;
                    ...
                    if (!matched && spin >= 0) {
                        std::string spin_suffix = "|" + std::to_string(spin);
                        auto it = pending_instant_spin_keys_.begin();
                        while (it != pending_instant_spin_keys_.end()) {
                            if (it->first.size() >= spin_suffix.size() &&
                                it->first.compare(it->first.size() - spin_suffix.size(), spin_suffix.size(), spin_suffix) == 0) {
                                if (now - it->second <= kWindow) {
                                    matched = true;
                                }
                                it = pending_instant_spin_keys_.erase(it);
                                break;
                            } else {
                                ++it;
                            }
                    ...
                    if (variant.size() >= 2) {
                        ...
                        }
                        {
                            ...
                            if (extract_spin(message, spin_name, spin_value, spin_key)) {
                                const bool show_real = host_flag("features.host.show_real_spin", true);
                                const bool show_qq = host_flag("features.host.show_qq_number", false);
                                const bool show_reme = host_flag("features.host.show_reme_spin", true);
                                const bool instant = host_flag("features.host.instant_roulette_spin", true);
                                const bool show_last = host_flag("features.host.show_last_spin", false);
                                ...
                                if (from_player_chat && spinner_netid != 0) {
                                    ...
                                    utils::PacketUtils::send_chat_message(const_cast<player::Player*>(&event.get_target()), message, false);
                                    ...
                                    if (spinner_netid != 0) {
                                        send_talkbubble_packet(spinner_netid, decorated_bubble);
                                        spdlog::info("[InstantSpin] Sent INSTANT talk bubble for netid {} ('{}')", spinner_netid, decorated_bubble);
                                    } else {
                                        auto norm_name = [](const std::string& s) {
                                            std::string out;
                                            for (unsigned char c : s)
                                        ...
                                        DeferredBubble db;
                                        db.normalized_name = norm_name(spin_name);
                                        db.text = decorated_bubble;
                                        db.spin_key = spin_key;
                                        db.spin_value = spin_value;
                                        db.queued_at = now;
                                        deferred_bubbles_.push_back(db);
                                        spdlog::info("[InstantSpin] Queued deferred instant bubble for '{}'", spin_name);
                                    }
                                    pending_instant_spin_keys_[spin_key] = now;
                                    pending_instant_spin_values_[spin_value] = now;
                ...
                if (function_name == "OnTalkBubble" && variant.size() >= 3) {
                    ...
                    uint32_t netid_early = read_netid_variant(variant, 1);
                    std::string spin_name, spin_key;
                    int spin_value = -1;
                    if (extract_spin(bubble_text, spin_name, spin_value, spin_key, netid_early)) {
                        const bool show_real = host_flag("features.host.show_real_spin", true);
                        const bool show_qq = host_flag("features.host.show_qq_number", false);
                        const bool show_reme = host_flag("features.host.show_reme_spin", true);
                        const bool instant = host_flag("features.host.instant_roulette_spin", true);
                        ...
                        if (instant && is_real) {
                            consume_pending_instant(spin_key, spin_value);
                            spdlog::info("[InstantSpin] Dropped server 3s delayed bubble for spin key='{}' val={}", spin_key, spin_value);
                            const_cast<core::EventPacket&>(event).canceled = true;
                            return;
                        }
    ...
    void track_player_positions(const core::EventPacket& event) {
        ...
        if (event.from == core::EventFrom::FromClient && game_packet.type == packet::PACKET_STATE) {
            ...
            if (tank) {
                ...
                spdlog::debug("[YOUR POSITION] Tile: ({}, {}) | Pixels: ({:.0f}, {:.0f})", 
    ...
    void handle_game_features(const core::EventPacket& event) {
        ...
        if (needs_modification) {
            ByteStream<std::uint16_t> byte_stream{};
            byte_stream.write(packet::NET_MESSAGE_GAME_PACKET);
            byte_stream.write(modified_tank);
            if (!ext_data.empty()) {
