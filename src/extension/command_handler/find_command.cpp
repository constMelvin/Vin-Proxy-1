...
#include "../item_finder/item_finder_extension.hpp"
...
void FindCommand::execute(client::Client* client, const std::vector<std::string>& args) {
    ...
    spdlog::info("FindCommand: Searching item database for '{}'", query);
    
    // Forward to ItemFinderExtension's GUI search
    auto* item_finder = g_core->query_extension<extension::item_finder::ItemFinderExtension>();
    
    if (item_finder) {
        item_finder->search_and_show_gui(server->get_player(), query);
    } else {
        // Fallback to vending search
