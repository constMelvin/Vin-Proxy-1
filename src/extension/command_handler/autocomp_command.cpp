...
void AutoCompCommand::execute(client::Client* , const std::vector<std::string>& ) {
    ...
    if (elapsed < 5) {
        send_console(server->get_player(), 
            fmt::format("`0[ `bVinProxy `0] `4Cooldown! Wait {} more seconds", 5 - elapsed));
        return;
    }

    if (s_running.exchange(true)) {
        send_console(server->get_player(), "`0[ `bVinProxy `0] `4AutoComp already running!");
        return;
    }
    ...
    send_console(server->get_player(), "`0[ `bVinProxy `0] `9Starting auto-compress...");
...
void AutoCompCommand::run_autocomp(std::uint64_t generation) {
    ...
    if (wl_count < WL_PER_DL) {
        ...
        send_console(server->get_player(), 
            fmt::format("`0[ `bVinProxy `0] `4Not enough WLs! (have: {}, need: {})", 
            wl_count, WL_PER_DL));
        finish();
        return;
    }
    ...
    if (new_wl_count == wl_count) {
        send_console(server->get_player(), 
            "`0[ `bVinProxy `0] `4Item locked by server! Wait 10-30 seconds before trying again.");
    ...
    send_console(server->get_player(),
        fmt::format("`0[ `bVinProxy `0] `2Compressed {} DL(s) successfully!", compressed));
