...
void HostCommand::send_settings_dialog(player::Player* out) {
    ...
    const int show_real = getb("features.host.show_real_spin", true) ? 1 : 0;
    const int show_qq = getb("features.host.show_qq_number", false) ? 1 : 0;
    const int show_reme = getb("features.host.show_reme_spin", true) ? 1 : 0;
    const int instant = getb("features.host.instant_roulette_spin", true) ? 1 : 0;
