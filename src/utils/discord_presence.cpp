...
void push_presence_rpc() {
    ...
    p.details = "License: Vin Proxy Premium";
    p.state = "Currently In-game";
    p.startTimestamp = g_started_at;
    p.endTimestamp = 0;
    p.largeImageKey = nullptr;
    p.largeImageText = "Visit Vin Proxy Premium";
    p.smallImageKey = nullptr;
    p.smallImageText = "Vin Proxy Premium";
    p.instance = 1;
    g_update_presence(&p);
}
...
bool push_presence_ipc() {
    ...
    nlohmann::json activity = {
        {"details", "License: Vin Proxy Premium"},
        {"state", "Currently In-game"},
        {"timestamps", {{"start", g_started_at}}},
        {"buttons", nlohmann::json::array({
            nlohmann::json{{"label", "Vin Proxy Premium Discord"}, {"url", "https://discord.gg/Cke6JZWhbN"}},
            nlohmann::json{{"label", "Vin Proxy Premium Website"}, {"url", "https://vinproxy.com"}}
        })}
    };
...
void DiscordPresence::start() {
    ...
    SetConsoleTitleA("Vin Proxy Premium");
    SetCurrentProcessExplicitAppUserModelID(L"com.vinproxy.desktop");
